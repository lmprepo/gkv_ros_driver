...
Принимающий узел (gkv_controller_ros_mode) считывает данные с выбранного пользователем serial-порта и передает их в топики, а также поднимает сервисы проверки соединения, сброса, установки алгоритма и запроса ID устройства. Устанавливает режим выдачи данных в формате стандартных ROS сообщений geometry_msgs/PoseStamped.
